...
//        //debag//if ((iCrcSummOneByteCompare(&m_puiRxBuffer[DATA_BYTE_OFFSET],
//                                    (DATA_EXCHANGE_COMMAND_ANSWER_LENGTH +
//                                     ONE_BYTE_CRC_LENGTH)) > 0) &&
//                ((m_puiRxBuffer[DATA_BYTE_OFFSET +
//                                DATA_EXCHANGE_COMMAND_ANSWER_LENGTH]) != 0))
        if ((iCrcSummOneByteCompare(&m_puiRxBuffer[DATA_BYTE_OFFSET],
                                    DATA_EXCHANGE_COMMAND_ANSWER_LENGTH) > 0))
        {
            m_uiBadAnswerCounter = 0;
...
//                            *puiDestination2++ = ((ucData & (1 << j)) ? 1 : 0);
                    }
                    else
                    {
