...
uint16_t CHammingCodes::BytesToHammingCodes(uint8_t* puiDestination, uint8_t* puiSourse, uint16_t uiLength)
{
    ...
    if (bTailIsPresent)
    {
        uint16_t uiData;
        // Получим кодовое слово 12 бит.
        uiData = static_cast<uint16_t>(enHamming(puiSourse[uiSourseIndex]));
...
uint16_t CHammingCodes::HammingCodesToBytes(uint8_t* puiDestination, uint8_t* puiSourse, uint16_t uiLength)
{
