...
__flash uint8_t CLightBoard::m_auiLedMap[] =
{
    ...
    // Например: Индекс окна - 6 сопоставлен с выходом OUT15 драйвера DD1 плата МРИ-21-24.
    // Индекс окна - 7 сопоставлен с выходом OUT2 драйвера DD1 плата МРИ-21-24.
...
void CLightBoard::Set(void)
{
    for (uint8_t i = 0;
            ...
            i++)
    {
        if (m_pxAlarmWindowControl[i].GetActivityState())
        {
